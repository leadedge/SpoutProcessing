...
public class SpoutReceiver {
	...
	public boolean createReceiver(String name) {
		...
		dim[0] = r_Parent.width; // app screen width
		dim[1] = r_Parent.height; // app screen height
		String newname;
	...
	public PGraphics receiveTexture(PGraphics pg)
	{
		...
		if(dim[0] != pg.width || dim[1] != pg.height && dim[0] > 0 && dim[1] > 0) {
			pg = r_Parent.createGraphics(dim[0], dim[1], PConstants.P2D);
		}
		else {    
	...
	//
	boolean checkReceiver(String name) {
